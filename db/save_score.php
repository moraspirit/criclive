...
if (strpos($score, ':') === false) {
    if ($cur_score_batsman == '') {
        $new_score_batsman = $score;
    } else {
        $new_score_batsman = $cur_score_batsman . ' ' . $score;
    }

    if ($cur_score_bowler == '') {
        $new_score_bowler = $score;
    } else {
        $new_score_bowler = $cur_score_bowler . ' ' . $score;
    }
    ...
    } else if ($score == '1' || $score == '3' || $score == '5' || $score == '7') {
...
//    }
    ...
    if ($cur_score_extra == '') {
        $new_score_extra = $extra_score . '' . $extra_type;
    } else {
        $new_score_extra = $cur_score_extra . ' ' . $extra_score . '' . $extra_type;
    }
...
//            }
            ...
            $result = mysqli_query($con, "UPDATE batsman SET status='3' WHERE matchid='$matchid' AND inning='$inning' AND status='1'");
            if ($result) {
                $result = mysqli_query($con, "UPDATE bowler SET score='$new_score_bowler' WHERE matchid='$matchid' AND inning='$inning' AND status='1'");
                if ($result) {
                    $result = mysqli_query($con, "UPDATE batsman SET status='1' WHERE matchid='$matchid' AND inning='$inning' AND status='2'");
